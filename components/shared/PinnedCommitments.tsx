...
export function PinnedCommitments({ games, pins = [], pinnedIds, onSelect, onUnpin, compact = false, emptySlotLabel = "Empty slot" }: Props) {
    ...
    <section className={compact ? styles.compact : styles.panel} aria-label="Games you have committed to">
      ...
      <ul className={styles.list}>
        ...
        {Array.from({ length: Math.max(0, 3 - pinned.length) }, (_, index) => (
          <li key={`empty-${index}`} className={styles.emptySlot}>{emptySlotLabel}</li>
        ))}
      </ul>
    </section>
  );
