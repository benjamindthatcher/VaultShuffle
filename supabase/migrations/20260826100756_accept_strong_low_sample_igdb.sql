-- Accept a conservative low-sample direct IGDB estimate. Identity must come
-- from IGDB's unique Steam external-game mapping, the row must have 2-4
-- submissions and at least two coherent duration tiers, and reused provider
-- identities or derivative product titles remain review-only.
...
create temporary table pg_temp.strong_low_igdb_protected_before
on commit drop
as
...
create temporary table pg_temp.strong_low_igdb_evidence_guard
on commit drop
as
select
...
create temporary table pg_temp.strong_low_igdb_quarantine_guard
on commit drop
as
select
...
comment on function public.reconcile_catalogue_duration(bigint, boolean) is
  'Projects hardened evidence plus narrowly accepted low-sample exact-Steam HLTB and direct IGDB estimates; preserves manual, excluded and nonfinite rows.';

create temporary table pg_temp.strong_low_igdb_plan
on commit drop
as
...
where game.duration_status = 'review_required'
  ...
  and (
    coalesce(estimate.main_story_minutes, estimate.main_extra_minutes) is null
    or estimate.completionist_minutes is null
    or estimate.completionist_minutes::bigint < coalesce(
      estimate.main_story_minutes,
      estimate.main_extra_minutes
...
alter table pg_temp.strong_low_igdb_plan
...
begin
  ...
  from pg_temp.strong_low_igdb_plan;

  if planned_count <> 744 then
    raise exception 'strong low-sample IGDB plan changed: expected 744, found %',
      planned_count;
  end if;
...
with desired as (
  ...
  from pg_temp.strong_low_igdb_plan as plan
  join public.catalog_games as game
...
begin
  if exists (
    ...
    raise exception 'a strong low-sample IGDB candidate did not resolve finite';
  end if;
  ...
  if exists (
    ...
    raise exception 'a strong low-sample IGDB candidate job did not complete';
  end if;

  if exists (
    select 1
    from pg_temp.strong_low_igdb_protected_before as before
    ...
    raise exception 'strong low-sample IGDB reconciliation changed a protected row';
  end if;

  if exists (
    select 1
    from pg_temp.strong_low_igdb_evidence_guard as before
    cross join lateral (
      select
        count(*) as estimate_rows,
        coalesce(sum(pg_catalog.hashtextextended(
    ...
    raise exception 'strong low-sample IGDB reconciliation changed raw evidence';
  end if;

  if exists (
    select 1
    from pg_temp.strong_low_igdb_quarantine_guard as before
    cross join lateral (
      select
        count(*) as quarantine_rows,
        coalesce(sum(pg_catalog.hashtextextended(
    ...
    raise exception 'strong low-sample IGDB reconciliation changed quarantine state';
  end if;
end;
$$;
