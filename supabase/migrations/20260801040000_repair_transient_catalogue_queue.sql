-- The previous catalogue worker could combine an app-reviews response with a
-- failed Store app-details request. That produced misleading "missing title"
-- and generic errors for healthy AppIDs. Re-open only those legacy failures;
-- the corrected worker now distinguishes transient request failures from an
-- authoritative Store `success: false` response.
...
comment on function public.queue_stale_catalogue_metadata(integer, interval, interval) is
  'Queues bounded stale/incomplete Steam metadata and backs off only AppIDs the Store authoritatively reports as unavailable.';
