-- Some retired/delisted AppIDs no longer return a Steam store payload. Their
-- existing shared catalogue records are still useful, but repeatedly queuing
-- them cannot add metadata and wastes most of a bounded worker run.
...
where status = 'pending'
  and attempts >= 5
  and last_error = 'Steam metadata did not include a title.';
  ...
  with candidates as (
    ...
    and not exists (
      ...
      where exhausted.steam_appid = game.steam_appid
        ...
        and exhausted.last_error = 'Steam metadata did not include a title.'
...
comment on function public.queue_stale_catalogue_metadata(integer, interval, interval) is
  'Queues a bounded, priority-ordered set of stale/incomplete Steam rows while backing off unavailable retired AppIDs.';
