-- Steam removes or suppresses metadata for some historical AppIDs. Preserve
-- the last useful shared catalogue snapshot instead of treating those rows as
-- failed forever.
update public.steam_app_metadata m
set title = coalesce(nullif(m.title, ''), c.name),
    genre = case
    ...
    rating = case
      when coalesce(m.rating, 0) > 0 then m.rating
    ...
    capsule_url = coalesce(m.capsule_url, c.capsule_url),
    header_url = coalesce(m.header_url, c.header_url),
    ...
    is_free = coalesce(m.is_free, false) or coalesce(c.is_free, false),
    status = 'ready',
    failure_count = 0,
    last_error = null,
    checked_at = now()
